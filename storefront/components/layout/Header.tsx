...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50">
          ...
          <div className="container mx-auto px-4 lg:px-12 max-w-[1400px]">
            <div className="flex items-center justify-between h-10 text-xs font-semibold">
              <div className="top-bar__contact flex items-center gap-6">
                <a href="tel:+48500169060" className="top-bar__phone flex items-center gap-1.5 hover:text-primary-500 transition-colors">
              ...
              <div className="top-bar__actions flex items-center gap-6">
                <Link href={`/${locale}/faq`} className="top-bar__faq hover:text-primary-500 transition-colors flex items-center gap-1.5">
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  FAQ
                </Link>
                <Link href={`/${locale}/tracking`} className="top-bar__tracking flex items-center gap-1.5 hover:text-primary-500 transition-colors">
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* Main Header - Induxter Orange Style */}
      <div className="main-header w-full bg-primary-500">
        <div className="mx-auto px-6 lg:px-12 max-w-[1400px]">
          <div className="main-header__content flex items-center justify-between h-16">
            ...
            <nav className="main-nav hidden lg:flex items-center gap-1">
              <span className="nav__separator text-white/50 text-lg font-light px-2">/</span>
              <Link href={`/${locale}/about`} className="nav-item text-white font-bold hover:text-secondary-700 transition-colors px-4 py-2 text-sm uppercase font-heading">
                ABOUT
              </Link>
              <span className="nav__separator text-white/50 text-lg font-light px-2">/</span>
              <Link href={`/${locale}/promotions`} className="nav-item text-white font-bold hover:text-secondary-700 transition-colors px-4 py-2 text-sm uppercase font-heading">
                PROMOTIONS
              </Link>
              <span className="nav__separator text-white/50 text-lg font-light px-2">/</span>
              <Link href={`/${locale}/contact`} className="nav-item text-white font-bold hover:text-secondary-700 transition-colors px-4 py-2 text-sm uppercase font-heading">
                CONTACT
              </Link>
            </nav>

            <div className="main-header__actions flex items-center gap-4">
              <Link href={`/${locale}/login`} className="main-header__account text-white hover:text-secondary-700 transition-colors">
