...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  // Filtruj notatki - pokaż tylko te do których użytkownik ma dostęp
  let filteredNotes = notes
  if (user_id) {
    filteredNotes = notes.filter((note: any) => 
      note.created_by === user_id || 
      !note.shared_with || // null = prywatna, tylko twórca
      note.shared_with.includes(user_id) ||
      note.shared_with.includes("all") // udostępniona wszystkim
    )
  }
